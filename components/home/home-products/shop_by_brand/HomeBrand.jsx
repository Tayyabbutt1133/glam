...
const HomeBrand = () => {
  const products = [
    { id: 1009, name: 'CLINIQUE', description: 'Discover Clinique\'s Anti-Wrinkle sunscreen', image: brand_one, brandLanding: 'clinique', brandListing: '1009' },
    { id: 1341, name: 'ESTEE LAUDER', description: 'Discover Estee Lauder\'s luxurious skincare', image: brand_two, brandLanding: 'estee-lauder', brandListing: '1341' },
    { id: 991, name: 'SCHWARZKOPF', description: 'Explore Schwarzkopf\'s professional hair care', image: brand_three, brandLanding: 'schwarzkopf', brandListing: '991' },
    { id: 1011, name: 'JIMMY CHOO', description: 'Experience Jimmy Choo\'s iconic fragrances', image: brand_four, brandLanding: 'jimmy-choo', brandListing: '1011' },
  ];
            ...
            >
              <div className="relative sm:w-full h-[120px] sm:h-72 2xl:h-[390px] w-[150px]">
                <Image 
                  ...
                  objectFit="cover" 
                />
                <h3 className={`sm:hidden text-md absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 font-semibold text-gray-50 ${jost.className}`}>
                  {product.name}
                </h3>
              </div>

              <div className="px-4 py-6 text-left">
                <h3 className={`hidden sm:block text-lg font-semibold text-gray-900 ${jost.className}`}>
                  {product.name}
                </h3>
                <p className={`text-black text-xs sm:text-sm my-2 ${lexendDeca.className}`}>
                  {product.description}
                </p>
