...
export default function Component() {
  ...
  // Fetching Products from the base URL
  ...
  const getFilteredCount = (filterType, value) => {
    return products.filter(product => {
      const brandMatch = filters.brands.length === 0 || 
        product.attributes.some(attr => 
          attr.name === "Brand" && (filters.brands.includes(attr.options[0]) || attr.options[0] === value)
        )
      const categoryMatch = filters.categories.length === 0 || 
        product.categories.some(cat => filters.categories.includes(cat.id.toString()) || cat.id.toString() === value)
      const priceMatch = filters.priceRange.length === 0 || 
        filters.priceRange.some(range => {
          const [min, max] = range.split('-').map(Number)
          const price = parseFloat(product.price)
      ...
      if (filterType === 'brands') {
        return categoryMatch && priceMatch && (
          product.attributes.some(attr => attr.name === "Brand" && attr.options.includes(value))
        )
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-32 mt-32">
        <div className="w-full lg:w-1/4 p-4 sticky top-0 bg-white">
          ...
          <div className="mb-6 mt-4">
            ...
            {isBrandFilterOpen && (
              <div className={`pl-2 ${lexendDeca.className} font-normal max-h-60 overflow-y-auto`}>
                {brands
