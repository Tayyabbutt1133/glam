...
//Adapter functions to normalize data from different schemas
const normalizeProduct = (product) => {
  // Check if product uses GraphQL schema (has nodes)
  const isGraphQLSchema = product.attributes?.nodes !== undefined;
  ...
  return {
    ...
    attributes: normalizeAttributes(product.attributes, isGraphQLSchema),
  };
};
...
const getBrand = (product) => {
  const isGraphQLSchema = product.attributes?.nodes !== undefined;
  const attributes = isGraphQLSchema
    ? product.attributes?.nodes
    : product.attributes;

  const brandAttribute = attributes?.find(
    (attr) => attr.name === (isGraphQLSchema ? "pa_brand" : "Brand"),
  );
...
export default function MyBag() {
  ...
  const handleEdit = (normalizedItem) => {
              ...
              {cartItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <div className="w-[120px] md:w-48 md:flex-shrink-0">
                      <Image
                        src={normalizedItem.image?.src || "/placeholder.svg"}
                    ...
                    <div className="ml-4 md:flex-grow">
                      {editingItem?.id === normalizedItem.id ? (
                        <div>
                          <input
                            type="text"
                            value={
                              editingItem
                                ? decodeHTMLEntities(editingItem.name)
                                : ""
                            }
                            onChange={(e) =>
                              setEditingItem({
                        ...
                        <>
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <Link href={`/product/${normalizedItem.id}`}>
                                <h2
                                  className={`2xl:text-[24px] xs:text-lg text-[16px] cursor-pointer font-medium w-[80%] ${jost.className}`}
                                >
                              ...
                              {normalizedItem.attributes.map((attr) => (
                                <p
                                  key={attr.name}
                                  className={`text-sm 2xl:text-[20px] text-black ${jost.className} font-normal mt-[10px]`}
                                >
                                  {attr.name}:{" "}
                                  {decodeHTMLEntities(attr.options[0] || "N/A")}
                                </p>
                              ))}
                              <p
                                className={`font-medium sm:hidden mt-4 sm:mt-0 block text-[16px] ${jost.className}`}
                              >
                                {currencySymbol}
                                {parseFloat(
                                  normalizedItem.price * rate * item.quantity,
                                ).toFixed(2)}
                              </p>
                            </div>
                            <p
                          ...
                          <div className=" -mt-12 sm:-mt-0 flex sm:flex-row flex-col sm:items-center items-end gap-4 justify-between ">
                            ...
                            <div className="space-x-8">
                              <button
                                className={`font-medium 2xl:text-[20px] text-black ${jost.className}`}
                                onClick={() =>
                                  handleSaveForLater(normalizedItem.id)
                                }
                              >
                                Save For Later
                              </button>
                              <button
                                className={`font-medium 2xl:text-[20px] text-black ${jost.className}`}
                                onClick={() => handleEdit(normalizedItem)}
                              >
                                Edit
                              </button>
                              <button
                                className={`font-medium 2xl:text-[20px] text-black ${jost.className}`}
                                onClick={() =>
                                  removeFromCart(normalizedItem.id)
                                }
                              >
                                Remove
                              </button>
