    ...
    <div>
      ...
      <section className=" py-4 lg:py-7">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-2">
            <h2
                          className={`text-2xl font-manrope uppercase font-semibold text-gray-900 leading-[3.25rem] ${jost.className}`}
            >
          ...
          <div className="accordion-group">
            {faqs.map((faq, index) => (
              <div
                key={index}
                className={`accordion py-8 px-6 border-b border-solid border-gray-200 transition-all duration-500 rounded-2xl hover:bg-indigo-50 ${openIndex === index ? 'accordion-active:bg-indigo-50 active' : ''}`}
              >
                <button
                ...
                >
                        <h5 className={`${lexendDeca.className} font-medium`}>{faq.question}</h5>
                  <svg
